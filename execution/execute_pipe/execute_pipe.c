...
static int fork_first_child(t_shell *shell, int pipefd[2], pid_t *pid1, t_ast *ast)
{
    *pid1 = fork();
    if (*pid1 == -1)
    {
...
static int fork_second_child(t_shell *shell, int pipefd[2], pid_t *pid2, t_ast *ast)
{
    *pid2 = fork();
    if (*pid2 == -1)
    {
